...
class QuizService:
    ...
    def generate_quiz_for_candidate(self, candidate_id: int, job_title: str, num_questions: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not candidate_skills:
                ...
                try:
                    ...
                    required_skills_with_levels = self._get_required_skills_for_job(job_title, db)
                    ...
                    if 'skills_quizzes' not in quiz_data:
                        raise ValueError("Invalid quiz format: missing 'skills_quizzes' key")
                ...
                for skill in candidate_skills:
                    ...
                    # Verify the quiz contains valid questions
                    if not quiz or not quiz.get("questions"):
                        raise ValueError(f"Failed to generate valid questions for skill: {skill}")
                    
                    num_questions = len(quiz["questions"])
                    if num_questions == 0:
                        raise ValueError(f"No questions generated for skill: {skill}")
            ...
            while retry_count <= max_retries and (not quiz_data or not quiz_data.get("questions")):
                try:
                    quiz_data = self.generator.generate_recruitment_quiz(
                        ...
                        num_questions=num_questions if retry_count == 0 else max(3, num_questions // 2),
    ...
    def _get_required_skills_for_job(self, job_title: str, db) -> List[Dict]:
        ...
        try:
            ...
            if not job:
                error_msg = f"❌ No job found with title: {job_title}"
                print(error_msg)
            ...
            if not formatted_skills:
                warning_msg = f"⚠️  No valid skills found for job: {job_title}. Using job title as fallback skill."
                print(warning_msg)
                return [{
            ...
            # Fallback to using job title as a skill
            print("⚠️  Falling back to using job title as skill")
            return [{
                'skill_name': job_title,
                'skill_level': 'intermediate',
                'is_required': True
