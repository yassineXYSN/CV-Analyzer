...
async def assign_job_quiz(job_id: int, request: Request, db: Session = Depends(get_db)):
    """Assign a quiz to a candidate without generating it - generation happens when they take it"""
    try:
        print(f"🔍 ASSIGNING QUIZ ONLY - No generation for job {job_id}")
        ...
        print(f"✅ Found job: {job.title} for candidate: {candidate_profile.id}")
        ...
        if existing_assignment:
            print(f"⚠️ Quiz already assigned for job {job_id}")
            return {"success": False, "message": "Un quiz est déjà assigné pour ce poste"}
        
        # Create job quiz assignment record WITHOUT generating quiz
        print(f"📝 Creating assignment record - NO QUIZ GENERATION")
        assignment = JobQuizAssignment(
            job_id=job_id,
            candidate_id=candidate_profile.id,
            assigned_by=None,
            status="assigned",  # Just assigned, not in progress yet
            quiz_attempt_id=None  # No quiz generated yet
        )
        db.add(assignment)
        db.commit()
        
        print(f"✅ Assignment created successfully - ID: {assignment.id}, Status: {assignment.status}")
        print(f"🔍 NO QUIZ CONTENT GENERATED - quiz_attempt_id is None")
        
        return {
            "success": True,
            "message": "Quiz assigné avec succès. Cliquez sur 'Passer quiz' pour commencer.",
            "assignment_id": assignment.id
        }
        
    except Exception as e:
        db.rollback()
        print(f"❌ Error assigning job quiz: {str(e)}")
        return {"success": False, "message": f"Erreur: {str(e)}"}
...
async def take_assigned_quiz(assignment_id: int, request: Request, db: Session = Depends(get_db)):
    """Take an assigned quiz - generates the quiz when candidate actually wants to take it"""
    try:
        print(f"🚀 TAKING QUIZ - Generating quiz content for assignment {assignment_id}")
        ...
        print(f"✅ Found assignment: Job {assignment.job_id}, Status: {assignment.status}")
        print(f"🔍 Quiz attempt ID before generation: {assignment.quiz_attempt_id}")
        
        # Generate quiz NOW when candidate wants to take it
        print(f"🎯 GENERATING QUIZ CONTENT NOW using QuizService")
        quiz_service = QuizService()
        ...
        if not quiz_data or not quiz_data.get("quiz_id"):
            print(f"❌ Quiz generation failed")
            return {"success": False, "message": "Erreur lors de la génération du quiz"}
        
        print(f"✅ Quiz generated successfully - ID: {quiz_data['quiz_id']}")
        
        # Update assignment with quiz attempt and mark as in progress
        assignment.quiz_attempt_id = quiz_data["quiz_id"]
        assignment.status = "in_progress"
        db.commit()
        
        print(f"📝 Assignment updated - quiz_attempt_id: {assignment.quiz_attempt_id}, status: {assignment.status}")
        ...
        print(f"❌ Error taking quiz: {str(e)}")
        return {"success": False, "message": f"Erreur: {str(e)}"}
...
async def submit_quiz(assignment_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Flatten answers shape: frontend sends {quizId: {questionId: letter}}
        flat_answers = answers
        try:
            if isinstance(answers, dict):
                # If keyed by quizId → extract the inner map for this attempt
                if any(isinstance(v, dict) for v in answers.values()):
                    key = str(assignment.quiz_attempt_id)
                    if key in answers and isinstance(answers[key], dict):
                        flat_answers = answers[key]
                    else:
                        # Take the first dict value as a fallback
                        first = next((v for v in answers.values() if isinstance(v, dict)), {})
                        flat_answers = first
        except Exception:
            flat_answers = answers
