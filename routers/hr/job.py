...
from databasehr.models import Job, Department, Employee, Application, ProfileCandidat, Contact, Company, HRAdmin, JobSkill
...
async def create_job(job_data: JobRequest, db: Session = Depends(get_db)):
    try:
...
async def get_job_details(job_id: int, db: Session = Depends(get_db)):
    try:
        ...
        # Get applications with recommendation info
