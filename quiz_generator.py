...
def query_deepseek(prompt):
    """Query the DeepSeek model using huggingface_hub.InferenceClient (aligned with provided structure)."""
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a quiz generator. Output ONLY valid JSON. Do not include <think> tags, markdown, or any extra text. For single-skill prompts, respond with a JSON array; otherwise respond with a JSON object. Start with '[' or '{' and end with ']' or '}'."
                },
                {
                    "role": "user",
                    "content": prompt
...
class QuizGenerator:
    ...
    def _parse_quiz_response(self, content: str, job_title: str = "") -> Dict[str, Any]:
        ...
        # Pre-clean: remove <think> blocks, code fences
        ...
        parsing_attempts = [
            ...
            # 4. Try finding any JSON array in the content
            lambda: try_parse_json(re.search(r'\[(?:[^\[\]]|\[(?:[^\[\]]|\[[^\[\]]*\])*\])*\]', content, re.DOTALL).group(0)
                                 if re.search(r'\[.*\]', content, re.DOTALL) else None),
            
            # 5. Try extracting questions using regex as last resort
            lambda: {
                'quiz_title': f"QCM - {job_title}" if job_title else "QCM",
                'job_title': job_title or "",
    ...
    def _generate_skill_quiz(self, job_title: str, skill: Dict, job_description: str = "", 
        ...
        """Generate a quiz for a specific skill using the expert prompt and robust parser."""
        ...
        try:
            prompt = self._create_expert_quiz_prompt(
                job_title=job_title,
                required_skills=[],
            ...
            parsed = self._parse_quiz_response(content, job_title)
            questions = parsed.get('questions', []) if isinstance(parsed, dict) else []
            if not questions:
                raise ValueError("Parsed response contains no questions")

            for q in questions:
                opts = q.get('options', {})
                if isinstance(opts, list):
                    keys = ['A','B','C','D'][:len(opts)]
                    q['options'] = {k: str(v) for k, v in zip(keys, opts)}
                correct = str(q.get('correct_answer', 'A')).strip().upper()
                if correct not in q['options']:
                    q['correct_answer'] = 'A'
                q.setdefault('explanation', '')
                q.setdefault('category', 'technique')
                q['skill_related'] = skill_name
                q['difficulty'] = skill_level
            ...
            if not quiz_data.get('questions'):
                raise ValueError("No valid questions after post-processing")
            print("✅ Successfully generated skill quiz using expert prompt")
            return quiz_data
        except Exception as e:
            print(f"❌ Skill quiz generation failed for {skill_name}: {e}")
            raise
    ...
    def generate_recruitment_quiz(self, job_title: str, required_skills_with_levels: List[Dict], 
        ...
        for skill in required_skills_with_levels:
            ...
            try:
                ...
                print(f"❌ Error generating quiz for {skill.get('skill_name', 'unknown')}: {str(e)}")
                raise
        # Also include a flattened questions list for compatibility with callers expecting it
        flat_questions = []
        for sq in quizzes['skills_quizzes']:
            qs = sq.get('questions') or []
            if isinstance(qs, list):
                flat_questions.extend(qs)
        quizzes['questions'] = flat_questions
        return quizzes
...
{{
                  "quiz_title": "{skill_name} ({skill_level})",
  "job_title": "{job_title}",
  "questions": [
    {{
      "id": 1,
...
- Follow the exact structure shown in the example"""
    ...
    def _parse_quiz_response_legacy(self, content: str, job_title: str = "") -> Dict[str, Any]:
        """Parse the raw response from the AI model into a structured quiz
        ...
        for question in questions:
            ...
            # Normalize options to a dict with A-D keys
            raw_options = question.get('options', [])
            option_texts = []
            if isinstance(raw_options, dict):
                if all(k.upper() in ['A','B','C','D'] for k in raw_options.keys()):
                    option_texts = [raw_options.get(k, '') for k in ['A','B','C','D']]
                else:
                    option_texts = list(raw_options.values())
            elif isinstance(raw_options, list):
                option_texts = raw_options
            else:
                option_texts = []
            option_texts = [str(opt).strip() for opt in option_texts if str(opt).strip()]
            if len(option_texts) < 2:
                continue
            option_texts = option_texts[:4]
            option_keys = ['A','B','C','D'][:len(option_texts)]
            options_dict = {k: v for k, v in zip(option_keys, option_texts)}
            
            # Normalize correct answer to letter A-D
            correct_answer = question.get('correct_answer', 'A')
            if isinstance(correct_answer, int):
                idx = max(0, min(correct_answer, len(option_texts)-1))
                correct_answer = chr(ord('A') + idx)
            else:
                correct_answer = str(correct_answer).strip().upper()
                if correct_answer not in options_dict:
                    # Try to map numeric strings
                    try:
    ...
    def evaluate_quiz(self, quiz_data: Dict[str, Any], answers: Dict[str, str]) -> Dict[str, Any]:
        ...
        for i, question in enumerate(quiz_data["questions"], 1):
            # Use provided question id if present; fallback to index
            raw_qid = question.get("id", i)
            question_id = str(raw_qid)
            correct_answer = str(question.get("correct_answer", "A")).strip().upper()
            user_answer = str(answers.get(str(question_id), "")).strip().upper()
