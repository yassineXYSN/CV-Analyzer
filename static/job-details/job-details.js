...
function renderCandidateActions(app) {
  ...
  function getQuizButtonHtml(app) {
    try {
      ...
      if (app.quiz_assignment_status) {
        ...
        const finalScore = typeof app.quiz_assignment_status.score === 'number' ? Math.round(app.quiz_assignment_status.score) : null;

        console.log("Quiz status:", { status, assignmentId, quizAttemptId, finalScore });
        ...
        if ((status === 'assigned' || status === 'in_progress') && assignmentId) {
          ...
          return `
            <button class="btn-action quiz completed" onclick="event.stopPropagation(); viewQuizResults('${quizAttemptId}')">
              <i class="fas fa-check-circle"></i> Quiz Complété${finalScore !== null ? ` — <strong>${finalScore}%</strong>` : ''}
            </button>
          `;
        }
      }
  ...
  function viewAssignedQuiz(assignmentId) {
    ...
    try {
      // Find the candidate for this assignment
      const app = applications.find(a => a.quiz_assignment_status && (a.quiz_assignment_status.assignment_id === assignmentId || a.quiz_assignment_status.id === assignmentId));
      const candidateId = app?.candidate_id || app?.id || '';
      const url = candidateId ? `/take-quiz/${candidateId}?assignment_id=${assignmentId}&job_id=${currentJob?.id || ''}&job_title=${encodeURIComponent(currentJob?.title || '')}` : `/take-quiz/${assignmentId}`;
      window.open(url, "_blank");
    } catch (e) {
      window.open(`/take-quiz/${assignmentId}`, "_blank");
    }
  }
...
async function assignQuizToCandidate(candidateId) {
  try {
    ...
    if (result.success) {
      ...
      // No redirection here; HR 'Évaluer Quiz' only assigns. Candidate will start from their 'Passer un QCM' flow.
    } else {
